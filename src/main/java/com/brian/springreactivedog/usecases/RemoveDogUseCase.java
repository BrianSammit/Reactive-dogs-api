...
public class RemoveDogUseCase {
    ...
    public Mono<DogDTO> subscribeClass(String dogId, String dogWlkId) {
        return repository
                ...
                .flatMap(dog -> {
                            if (!dog.getIsAdded()) {
                                return Mono.error(new IllegalArgumentException("The dog is not added"));
                            } else {
                                dog.setIsAdded(false);
                ...
                .doOnSuccess(dogDTO -> {
                    try {
                        dogPublisher.publishRemove(dogWlkId, dogDTO);
